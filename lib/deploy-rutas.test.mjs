...
test('en ESTE árbol, ninguna ruta que DEPLOY.md declara es invisible', (t) => {
  const p = join(ROOT, 'DEPLOY.md');
  if (!existsSync(p)) {
    // SKIP DECLARADO, no `return` silencioso. Un auditor de regresion cazo que
    // este caso pasaba 4/4 en un arbol sin DEPLOY.md sin afirmar NADA: verde
    // vacuo. Lo probo sembrando un DEPLOY.md con 13 rutas y un .gitignore que
    // ocultaba una -- el caso se puso rojo al instante, confirmando que el verde
    // anterior no medía nada.
    //
    // Es la misma distincion que el motor hace en la etapa 1 con memory/: «no
    // habia con que comprobar» y «comprobe y esta bien» son estados DISTINTOS, y
    // un `return` los colapsa en el segundo. Con `t.skip` el recuento lo dice.
    t.skip('sin DEPLOY.md en este arbol: no hay rutas declaradas que verificar');
    return;
  }
