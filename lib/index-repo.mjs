...
/**
 * La raiz contra la que se indexa. H2: el motor tiene que poder mirar un arbol
 * que no es el suyo -- bajo `node_modules/@harness/ring` el ROOT deducido apunta
 * al gestor de paquetes, y el anillo indexaria las dependencias.
 *
 * DEFENSIVA A PROPOSITO, igual que `raizDe` en artifact.mjs. Acepta la raiz
 * SUELTA (que es como la pasan `ficherosDeGit(root)` e `indexar({root})`) o
 * dentro de `{ raiz }`, y cualquier otra cosa cae al defecto deducido:
 *   - `.map(fn)` pasa el INDICE como segundo argumento: ya metio un indice como
 *     taxonomia en un refactor de este repo.
 *   - una raiz vacia compondria una ruta RELATIVA que resuelve contra el cwd del
 *     proceso, que dentro de un hook es otro: el modo de fallo que borro 1181
 *     ficheros.
 */
export const raizDeIndice = (r) => {
  if (typeof r === 'string' && r.trim()) return r;
  if (r && typeof r === 'object' && typeof r.raiz === 'string' && r.raiz.trim()) return r.raiz;
  return ROOT;
};

export function ficherosDeGit(root) {
  const raiz = raizDeIndice(root);
  const salida = execFileSync('git', ['ls-files', '-s'], {
    ...
    cwd: raiz, env: ENV_LIMPIO(), encoding: 'utf8', maxBuffer: 64 * 1024 * 1024,
  });
 ...
 */
export function indexar({ root, previo = leer() } = {}) {
  const raiz = raizDeIndice(root);
