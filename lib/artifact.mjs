...
export function writeArtifact(path, env, opciones) {
  ...
  const abs = path.startsWith('/') ? path : join(raizDe(opciones), path);
...
/**
 * Resuelve la raiz contra la que componer una ruta relativa.
 *
 * `opciones` es OPCIONAL y defensivo a proposito: si llega cualquier cosa que no
 * sea un objeto con `raiz` string no vacia, cae al ROOT deducido. Eso cubre el
 * defecto real que ya mordio en este repo --`.map(fn)` pasa el INDICE como
 * segundo argumento-- y tambien la raiz vacia, que compondria una ruta RELATIVA
 * y la resolveria contra el cwd del proceso. Dentro de un hook el cwd es otro:
 * ese es el modo de fallo que borro 1181 ficheros.
 */
const raizDe = (opciones) =>
  (opciones && typeof opciones === 'object' && typeof opciones.raiz === 'string' && opciones.raiz.trim())
    ? opciones.raiz
    : ROOT;

export function readArtifact(path, opciones) {
  const abs = path.startsWith('/') ? path : join(raizDe(opciones), path);
