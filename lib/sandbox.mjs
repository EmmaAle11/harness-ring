 ...
 */
/**
 * La raiz contra la que se evaluan los limites. H2: bajo
 * `node_modules/@harness/ring` el ROOT deducido apunta al gestor de paquetes, y
 * los guardarrailes protegerian el arbol equivocado.
 *
 * DEFENSIVA, igual que `raizDe` en artifact.mjs y `raizDeIndice` en
 * index-repo.mjs -- y aqui importa mas que en ningun otro sitio: si una raiz
 * basura colara, el limite se evaluaria contra una ruta vacia, que resuelve
 * contra el cwd del proceso. Ese es el modo de fallo que borro 1181 ficheros.
 *
 * NO RELAJA NADA. La primera guarda de `assertDestructible` exige que la ruta
 * este DENTRO de WORKSPACES, y eso no depende de la raiz: ninguna raiz inyectada
 * puede hacer destruible algo de fuera. La raiz solo decide QUE ARBOL es
 * intocable ADEMAS de los workspaces.
 */
export const raizSandbox = (r) => {
  if (typeof r === 'string' && r.trim()) return r;
  if (r && typeof r === 'object' && typeof r.raiz === 'string' && r.raiz.trim()) return r.raiz;
  return ROOT;
};

export function assertDestructible(path, opciones) {
  if (!path || typeof path !== 'string') throw new Error(`ruta invalida: ${path}`);
  const abs = resolve(path);
  const raiz = raizSandbox(opciones);

  if (!abs.startsWith(WORKSPACES + '/')) {
    throw new Error(`REHUSADO: '${abs}' esta fuera de ${WORKSPACES}. Solo se borran workspaces.`);
  }
  if (abs === WORKSPACES || abs === resolve(raiz) || abs === resolve('/')) {
...
export function assertOutsideMainTree(ws, opciones) {
  const abs = resolve(ws.path);
  const raiz = raizSandbox(opciones);
  if (abs.startsWith(resolve(raiz, 'src')) || abs.startsWith(resolve(raiz, 'backend', 'src'))) {
